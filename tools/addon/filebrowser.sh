...
DB_PATH="/usr/local/Dbz9/filebrowser.db"
DEFAULT_PORT=8080
...
if [[ "${install_prompt,,}" =~ ^(y|yes)$ ]]; then
    ...
    mkdir -p /usr/local/Dbz9
    chown root:root /usr/local/Dbz9
    chmod 755 /usr/local/Dbz9
    touch "$DB_PATH"
...
[Service]
User=root
WorkingDirectory=/usr/local/Dbz9
ExecStartPre=/bin/touch /usr/local/Dbz9/filebrowser.db
ExecStartPre=/usr/local/bin/filebrowser config set -a "0.0.0.0" -p ${PORT} -d /usr/local/Dbz9/filebrowser.db
ExecStart=/usr/local/bin/filebrowser -r / -d /usr/local/Dbz9/filebrowser.db -p ${PORT}
Restart=always
